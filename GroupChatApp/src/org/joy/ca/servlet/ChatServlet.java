 ...
 *
 */
@WebServlet(urlPatterns = { "/chat", "/chat/emoji", "/chat/recover",
		"/chat/allusers" })
public class ChatServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		if (CommonProcessor.checkLoggedInUserByToken(token,
			...
			case "allusers":
				...
				response = processor.getAllUsers(token);
				break;
			default:
				break;
			}
	...
	/**
	 * Load the chat page and set info of logged in user in request attribute
	 * for further processing in JSP.
	 */
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		boolean isValidUser = CommonProcessor.checkLoggedInUserByToken(token,
				session.getAttribute("token"));

		if (isValidUser) {
			for (UserInfo info : GlobalResources.USER_LIST) {
				if (info.getToken().equals(token)) {
					UserInfo userInfo = CommonProcessor
							.getUserDetailsByToken(formData.get("token"));
					req.setAttribute("user", userInfo);
					break;

				}
			}
			req.getRequestDispatcher(CommonResources.CLIENT_PAGE).forward(req,
					resp);
